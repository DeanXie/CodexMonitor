...
if (Test-Path -LiteralPath $cancelSignalPath) {
    throw "A3 CANCEL signal already exists"
}

$armed = [System.IO.File]::ReadAllText($armedPath) | ConvertFrom-Json
if ([string]$armed.state -ne "ARMED_WAITING_FOR_GO") {
    throw "A3 runner is not waiting for GO or CANCEL"
}
...
$tempPath = "$cancelSignalPath.tmp-$PID-$([guid]::NewGuid().ToString('N'))"
try {
    $json = $signal | ConvertTo-Json -Depth 8
    [System.IO.File]::WriteAllText($tempPath, $json, [System.Text.UTF8Encoding]::new($false))
    [System.IO.File]::Move($tempPath, $cancelSignalPath)
} finally {
...
Write-Host "A3 CANCEL signal created for runner PID $($armed.runnerPid)."
