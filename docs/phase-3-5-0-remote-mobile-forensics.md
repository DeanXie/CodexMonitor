...
The current daemon has one effective `CODEX_HOME`. Copying its complete data directory preserves the same logical `RemoteHostIdentity`; simultaneous cloned identities are an unsupported operational condition. Phase 3.5.1c provides typed, process-local Host availability without changing Thread authority, as documented in `phase-3-5-1c-host-availability.md`. Phase 3.5.1d Windows Remote Acceptance completed Gates A-L with canonical Thread, projection, Workspace/Project, token/runtime, and Host identity isolation intact. The corrective connection-ownership, endpoint-admission, and terminal-failure slices are PASS / FROZEN. Gate G's restricted-launch result is invalid acceptance-harness evidence; its production-lifecycle retest passed. Phase 3.5.2a is PASS / COMPLETE / FROZEN with exact read separated from writer admission. Phase 3.5.2b host-session writer admission observation, including slices b.1-b.5, is PASS / COMPLETE / FROZEN. Phase 3.5.2c subscription/release lifecycle, including slices c.1-c.5, is PASS / COMPLETE / FROZEN with its existing behavior frozen in fixtures and compatibility tests. Phase 3.5.2d.1 adds process-local Remote transport generations and compound request provenance without adding client or ownership identity. Phase 3.5.2d.2 binds daemon resume and upstream-unsubscribe requests to existing shared session attempt IDs without changing mutation, retry, replay, response, or event-routing semantics. Phase 3.5.2d.3 admits Remote notifications to the App event hub only from the fully ready current transport generation; stale notifications and stale teardown cannot mutate current availability or shared session truth. Phase 3.5.2d.4 adds a distinct daemon-process generation, preserves Host pinning, and requires explicit WorkspaceSession re-establishment after restart without inheriting old observation truth. Deferred Mobile Artifact Validation remains NOT YET EXECUTED.
