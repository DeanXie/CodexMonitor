...
Status: **FORENSICS COMPLETE**. Phase 3.5.1, Phase 3.5.2, Phase 3.5.3, Phase 3.5.4, and the Phase 3.5 Final Integration Acceptance are **PASS / COMPLETE / FROZEN**. Phase 3.5 is **PASS / COMPLETE / FROZEN**. Phase 4 is **IN PROGRESS** under the frozen P4.0 authority.
...
The current daemon has one effective `CODEX_HOME`. Copying its complete data directory preserves the same logical `RemoteHostIdentity`; simultaneous cloned identities are an unsupported operational condition. Phase 3.5.1c provides typed, process-local Host availability without changing Thread authority, as documented in `phase-3-5-1c-host-availability.md`. Phase 3.5.1d Windows Remote Acceptance completed Gates A-L with canonical Thread, projection, Workspace/Project, token/runtime, and Host identity isolation intact. The corrective connection-ownership, endpoint-admission, and terminal-failure slices are PASS / FROZEN. Gate G's restricted-launch result is invalid acceptance-harness evidence; its production-lifecycle retest passed. Phase 3.5.2a is PASS / COMPLETE / FROZEN with exact read separated from writer admission. Phase 3.5.2b host-session writer admission observation, including slices b.1-b.5, is PASS / COMPLETE / FROZEN. Phase 3.5.2c subscription/release lifecycle, including slices c.1-c.5, is PASS / COMPLETE / FROZEN with its existing behavior frozen in fixtures and compatibility tests. Phase 3.5.2d Remote transport/request coordination, including slices d.1-d.5, is PASS / COMPLETE / FROZEN. It separates Host, daemon-process, transport, WorkspaceSession, and app-server connection continuity; freezes transport-scoped request provenance, stale-delivery isolation, daemon-restart reset, multi-client shared-session semantics, and zero retry/replay behavior in sanitized fixtures and shared App/daemon tests. Phase 3.5.3 approval/delete authority, including slices a-e, is PASS / COMPLETE / FROZEN with exact approval identity, fake-only decision dispatch, exact-ID delete authority, unknown/stale multi-client isolation, confirmed-only tombstones, and aggregate compatibility fixtures. Phase 3.5.4a-e and the whole Phase 3.5.4 are PASS / COMPLETE / FROZEN with process-local coverage freshness, generation-tagged delivery, authoritative rehydration, offline/stale multi-client UI safety, aggregate compatibility fixtures, no telemetry persistence, and zero mutation retry/replay. The authenticated isolated read-only Final E2E proved authentication, Host validation, exact list/read, generation-tagged observation, transport-generation renewal, authoritative rehydration, and zero forbidden real mutations; its sanitized evidence is recorded in `docs/evidence/phase-3-5-final/`. Stale old-generation rejection in that record is deterministic fixture evidence backed by production-function regression coverage, not a real delayed old-socket notification E2E; that real transport scenario remains NOT_EXECUTED. Daemon event-stream completeness, exact upstream duplicate/late approval-response behavior, and exact upstream concurrent duplicate-delete ordering/winner semantics remain NOT PROVEN. Deferred Mobile Artifact Validation remains NOT YET EXECUTED.
