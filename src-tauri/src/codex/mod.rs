...
pub(crate) async fn spawn_workspace_session(
    ...
    let execution_settings_evidence = app_handle
        .state::<AppState>()
...
#[tauri::command]
pub(crate) async fn get_writer_admission_observation(
    workspace_id: String,
    thread_id: String,
    state: State<'_, AppState>,
    app: AppHandle,
) -> Result<
    crate::shared::codex_core::writer_admission_observation::WriterAdmissionObservationSnapshot,
    String,
> {
    if remote_backend::is_remote_mode(&*state).await {
...
#[tauri::command]
pub(crate) async fn thread_upstream_unsubscribe(
    workspace_id: String,
    thread_id: String,
    state: State<'_, AppState>,
    app: AppHandle,
    ...
    codex_core::thread_upstream_unsubscribe_core(&state.sessions, workspace_id, thread_id).await
}
