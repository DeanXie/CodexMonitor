...
Status: **FORENSICS COMPLETE**. Phase 3.5 is **IN PROGRESS**. Phase 3.5.1a Mobile Compile Boundary is **PASS / FROZEN (Windows implementation scope)**. Phase 3.5.1b RemoteHostIdentity and Phase 3.5.1c Host Availability are **PASS / FROZEN**. Phase 3.5.1d Windows Remote Acceptance is **PASS / COMPLETE**; Gates A-L are **PASS**. Phase 3.5.1 is **PASS / COMPLETE**. Phase 3.5.2a Exact-ID Remote Read / Admission Separation, Phase 3.5.2b host-session writer admission observation (slices b.1-b.5), Phase 3.5.2c subscription/release lifecycle (slices c.1-c.5), and Phase 3.5.2d Remote transport/request coordination (slices d.1-d.5) are **PASS / COMPLETE / FROZEN**. Phase 3.5.3a-e and the whole Phase 3.5.3 Approval and Delete Authority are **PASS / COMPLETE / FROZEN**. Phase 3.5.4a Projection Freshness Authority, Phase 3.5.4b Generation-tagged Event Delivery, Phase 3.5.4c Authoritative Recovery / Hydration, and Phase 3.5.4d Offline / Stale UI & Multi-client Isolation are **PASS / COMPLETE / FROZEN**; Phase 3.5.4 remains **IN PROGRESS** and Phase 3.5.4e is not started.
...
The current daemon has one effective `CODEX_HOME`. Copying its complete data directory preserves the same logical `RemoteHostIdentity`; simultaneous cloned identities are an unsupported operational condition. Phase 3.5.1c provides typed, process-local Host availability without changing Thread authority, as documented in `phase-3-5-1c-host-availability.md`. Phase 3.5.1d Windows Remote Acceptance completed Gates A-L with canonical Thread, projection, Workspace/Project, token/runtime, and Host identity isolation intact. The corrective connection-ownership, endpoint-admission, and terminal-failure slices are PASS / FROZEN. Gate G's restricted-launch result is invalid acceptance-harness evidence; its production-lifecycle retest passed. Phase 3.5.2a is PASS / COMPLETE / FROZEN with exact read separated from writer admission. Phase 3.5.2b host-session writer admission observation, including slices b.1-b.5, is PASS / COMPLETE / FROZEN. Phase 3.5.2c subscription/release lifecycle, including slices c.1-c.5, is PASS / COMPLETE / FROZEN with its existing behavior frozen in fixtures and compatibility tests. Phase 3.5.2d Remote transport/request coordination, including slices d.1-d.5, is PASS / COMPLETE / FROZEN. It separates Host, daemon-process, transport, WorkspaceSession, and app-server connection continuity; freezes transport-scoped request provenance, stale-delivery isolation, daemon-restart reset, multi-client shared-session semantics, and zero retry/replay behavior in sanitized fixtures and shared App/daemon tests. Phase 3.5.3 approval/delete authority, including slices a-e, is PASS / COMPLETE / FROZEN with exact approval identity, fake-only decision dispatch, exact-ID delete authority, unknown/stale multi-client isolation, confirmed-only tombstones, and aggregate compatibility fixtures. Phase 3.5.4a is PASS / COMPLETE / FROZEN with process-local, generation-aware, coverage-scoped projection freshness, authoritative read instrumentation, and shared App/daemon read-only parity. Phase 3.5.4b is PASS / COMPLETE / FROZEN with session/connection-tagged shared events, Remote delivery-time daemon/transport binding, and fail-closed frontend generation admission. Deferred Mobile Artifact Validation remains NOT YET EXECUTED.
