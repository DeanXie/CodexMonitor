...
- Debug panel copy/clear, sound notifications, platform-specific window effects (macOS overlay title bar + vibrancy), and a reduced transparency toggle. In-app updating and third-party Sentry upload are disabled for the first DeanX daily-use distribution.
...
- Threads/turns/reviews: `start_thread`, `fork_thread`, `compact_thread`, `list_threads`, `resume_thread`, `delete_thread`, `archive_thread`, `set_thread_name`, `send_user_message`, `turn_interrupt`, `respond_to_server_request`, `start_review`, `remember_approval_rule`, `get_commit_message_prompt`, `generate_commit_message`, `generate_run_metadata`.
