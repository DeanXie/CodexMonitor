...
export function ModelUsageBreakdown({ snapshot, models: filteredModels }: { snapshot: LocalUsageSnapshot | null; models?: AgentMonitorModelUsage[] | null }) {
  ...
  const sourceLabel = filteredModels ? "Selected session history" : "Local session history";
  return <section className="agent-monitor-models" aria-label="Historical model usage">
    <div className="agent-monitor-section-heading"><h2>Historical model usage</h2><span>{sourceLabel}</span></div>
