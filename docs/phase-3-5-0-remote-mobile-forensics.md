...
The current daemon has one effective `CODEX_HOME`. Copying its complete data directory preserves the same logical `RemoteHostIdentity`; simultaneous cloned identities are an unsupported operational condition. Phase 3.5.1c provides typed, process-local Host availability without changing Thread authority, as documented in `phase-3-5-1c-host-availability.md`. Phase 3.5.1d Windows Remote Acceptance is in progress with Gates A-H **PASS**. Single-flight and generation-aware cache ownership resolve the concurrent reconnect race; bounded backend negative-result coalescing prevents sequential endpoint reconnect storms while preserving `ENDPOINT_UNREACHABLE` as current truth; authoritative authentication and daemon-validation failures survive same-attempt cleanup disconnect without becoming sticky across new attempts. The next acceptance step is Gate I.
